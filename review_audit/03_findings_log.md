...
| F-047 | Segurança / Segredos | Crítico | P0 | Confirmado | Firebase API key `[valor redigido]` exposta em `firebase-applet-config.json` commitado publicamente | `firebase-applet-config.json:4`, `.gitignore` não cobre JSON | Rotacionar key, restringir no Google Cloud Console, adicionar Firebase App Check |
